...
import (
	...
	"github.com/openshift-hyperfleet/adapter-validation-gcp/status-reporter/pkg/reporter/testhelpers"
...
var _ = Describe("Reporter", func() {
	...
	Describe("Constants", func() {
		It("exports expected constant values", func() {
			Expect(reporter.DefaultConditionType).To(Equal("Available"))
	...
	Describe("updateFromTerminatedContainer", func() {
		...
		Context("when container exited with zero code", func() {
			...
			// Therefore, we treat this as a crash/failure and mark it as AdapterCrashed.
			It("updates with AdapterCrashed reason when no result file was produced", func() {
				terminated := &corev1.ContainerStateTerminated{
					Reason:   "Completed",
					ExitCode: 0,
				}
				...
				Expect(mock.LastUpdatedCondition.Type).To(Equal("Available"))
				Expect(mock.LastUpdatedCondition.Status).To(Equal("False"))
				Expect(mock.LastUpdatedCondition.Reason).To(Equal(reporter.ReasonAdapterCrashed))
				Expect(mock.LastUpdatedCondition.Message).To(ContainSubstring("Completed"))
			})
		})
